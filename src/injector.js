...
import { MODULE_NAME, getSettings } from './settings.js?rmv=1.5.39-tttouch1';
import {
...
import { recordRabbitMirrorInjection, recordRabbitMirrorNoInjection } from './tokenMeter.js?rmv=1.5.39-tttouch1';
import { getCurrentChatKey, markPendingBatchAttempt, releasePendingComboBatch } from './storage.js?rmv=1.5.39-tttouch1';
import { describeExternalWorldBookPreflightFailure } from './externalWorldBook/errors.js?rmv=1.5.39-tttouch1';
...
function loadPromptBuilder() {
    if (!promptBuilderPromise) {
        promptBuilderPromise = import('./promptBuilder.js?rmv=1.5.39-tttouch1').catch(error => {
...
function loadGenerationGuard() {
    if (!generationGuardPromise) {
        generationGuardPromise = import('./generationGuard.js?rmv=1.5.39-tttouch1').catch(error => {
...
export async function rabbitMirrorGenerateInterceptor(_chat, _contextSize, _abort, type) {
    ...
    try {
        if (materialEnabled) {
            ...
            let repository;
            if (externalEnabled) {
                repository = await import('./externalWorldBook/store.js?rmv=1.5.39-tttouch1');
                assertFollowPrefetchOwner(prefetchOwner, _chat);
                externalStage = 'index';
            ...
            if (frozenPlan.appearanceReference.enabled) {
                ...
                const appearance = await import('./appearanceReference.js?rmv=1.5.39-tttouch1');
