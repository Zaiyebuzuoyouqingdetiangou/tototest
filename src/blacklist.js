import { getSettings, updateSettings } from './settings.js?rmv=1.3.66';
...
// 1.3.66: entering a long chat can ask for the recipe of many mirrors in one install pass.
...
export function toggleBlacklistItem(kind, id) {
    // 1.3.66: 原本无论 add/remove 是否真的成功都固定返回 true/false，调用方据此弹
...
export function getRabbitMirrorRecipe({ chatKey = '', messageIndex = -1, swipeId = -1, message = null } = {}) {
    ...
    if (Number.isInteger(swipe) && swipe >= 0) {
        ...
        // 1.3.66: 原本在找不到精确记录时会回落到「同一条消息的任意 swipe」，于是新
...
export function blacklistPoolStats() {
    ...
    return {
        ...
        themeBlocked,
        formatBlocked,
        // 1.3.66: 设置页的「候选已全部拉黑」警告改用实际随机池判断。当前 allowByMode
