...
import { EXTERNAL_WORLD_BOOK_MAX_FILE_BYTES } from './schema.js?rmv=1.5.37-update1';
import { normalizeFileWorldBook } from './normalize.js?rmv=1.5.37-update1';
...
async function readLocalFile(file, allowLibraryBackup = false) {
    ...
    if (allowLibraryBackup && raw?.format === 'RabbitMirror.ExternalLibraries') {
        const { validateExternalLibraryBackup } = await import('./backup.js?rmv=1.5.37-update1');
