...
- 外置兔子镜整体左右留白由约 12px/侧调整为约 16px/侧，标题栏、外框与内容区继续作为整体等比例向内收。
- 保持外置宽度与 SillyTavern 美化正文容器解耦，不再依赖 `.mes_text` / `.mes_block` 的结构或 padding。
- inline / 外置后内嵌继续保持原有正文容器自适配，不应用本次外置留白。
- 未修改 Prompt、随机抽取、独立 API 请求、外置一体化、历史顶层弹窗与维修兔逻辑。
