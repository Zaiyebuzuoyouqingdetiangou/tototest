...
- 副 API Visual Scenery 配色去重改为“真实成品检测 → 仅在连续综合色调骨架重复时临时纠偏”。不设固定目标色、颜色名单或轮换表，仍由当前场景、材质、环境、光线与叙事自行决定配色。
- 动态视觉不再继承共享的五轮“低明度 → 中/高明度”配色冷却，避免副 API 被持续推向白色、浅灰、浅暖色安全区；主 API 与非 Visual Scenery 路线保持原逻辑。
- Visual Scenery 重复检测直接复用 visualScanner 的整体配色骨架相似度（主承载底色、大面积结构色、明暗关系与强调色），并只比较最近的副 API Visual Scenery 实际成品。
- 命中重复时只增加一条短暂的本轮纠偏；未命中时不增加任何配色 Prompt。

# 1.2.67

- 主 API 本地配色去重从“单一综合色调/主色”升级为“整体配色骨架”：视觉扫描器同时记录主承载底色、大面积第二结构色、明暗对比关系与低权重强调色证据；即使两面因为黑色面板与浅色纸面占比不同而得到不同 aggregate 主色，只要整体底盘仍高度相似，也会判为连续重复。
- 去重仍是通用规则，不针对“米纸 + 黑灰 + 红点缀”写死；粉/绿/蓝/紫/橙等任意主底色与结构组合都会用同一套骨架相似度比较。旧的单一主色相似度保留作 fallback，因此“连续两面都是粉色/绿色”等仍可命中。
- 清洗器在骨架重复时优先改主承载底色，而不是跟着 aggregate 面积赢家去改黑面板；目标色会同时避开当前与近期骨架中已经使用的主要色相家族。重映射按每个实际表面的原明暗带计算，避免浅色表面因整面被判 dark 而被错误压成深色。
- 改色后重新扫描“整体配色骨架”；只要仍与近期成品重复就撤回。诊断新增骨架签名，便于确认是主底、结构色还是强调关系造成重复。
- `promptBuilder` 与独立 API 功能内容未改，仅更新缓存版本号；本版不增加 Prompt 文本。材质/环境/光线决定配色的原美化规则继续保留，固有材质保护继续生效。

# 1.2.65

- 主 API 本地配色去重从“只处理连续米白/暖灰”扩展为真正的主色家族去重：当前面只要与上一面高置信连续撞同一综合色调家族，就进入安全重映射，不再等待第三面。
- 视觉扫描器保留低彩度但可靠的粉、绿、蓝、紫等真实色相，并增加主综合色相角记录；不再把所有低彩度成品一律压成 `muted`，避免淡粉/淡绿也被误视为同一灰白家族。
- 清洗器取消 1.2.64 的 13%～22% 低饱和、72%～91% 高明度固定目标区间；重复时从近期未使用的粉/绿/蓝/紫/青/橙/红/黄家族中选择新主色，同时尽量保留原材质表面的明暗关系、透明度和层次，只处理占画面较大的主题承载面；明确标记为原木、皮肤、血迹、火焰、树叶、石材、皮革、铜/金等固有材质的表面会跳过本地改色。
- 黑/白/米黄均不永久禁用；只阻止连续主色家族重复。改色后重新扫描，若仍与上一面相似或未形成新的可靠主色家族则自动撤回。独立 API 配色链与美化 Prompt 未修改，本版不增加任何 Prompt 文本。

# 1.2.64

- 配色重复治理移到本地检测/清洗链：主 API 实际成品按当前聊天记录综合色调指纹，不向 Prompt 增加任何配色规则。
- 连续两面历史成品与当前面都属于“浅亮 + 低饱和 + 暖/中性”且高置信相似时，只对最新主 API 兔子镜的大面积承载背景做本地轻量重映射；剧情强调色、SVG、图片和小面积物件不改。
- 本地改色后重新扫描验证；未成功脱离重复家族或意外变暗时自动撤回。副 API 不进入这条自动重映射链，保留现有独立配色策略。
- 全链路诊断新增本地配色家族、重复判定、重映射状态与原因。

## 1.2.63

- 维修兔 v2.16：新增“固定 ID 多阶段脚本时间线”安全恢复。宿主删除 `<script>` 后，可在不执行模型原脚本的前提下，恢复固定元素点击、固定样式/文字变更、固定 `setTimeout` 阶段以及明确的二次点击复原。
- 新路线仅接受受限语法；遇到动态选择器、未知语句、任意函数调用、网络/脚本执行能力等即整段拒绝，不做部分执行。
- 全链路诊断新增“脚本时间线恢复”计数，并将可安全恢复但尚未接管的时间线列为交互维修项。

## 1.2.62
- 跟随主 API 的外置兔子镜在 PC 端改用与独立 API 外置相同的宽度策略：`width: calc(100% - 44px)`、`max-width: 560px`、居中显示；手机端同样使用与独立 API 一致的左右留白宽度。仅统一外置 shell 几何，不改生成链、锚点、内嵌模式或 Prompt。

## 1.2.60
- 修复跟随主 API 的 Visual Scenery 缺少 near-output 最终核对：仅在本轮实际为 Visual Scenery 时，于跟随主 API 注入末尾追加一条短动态验收，不影响普通展现形式。
- 最终核对要求主要场景在未操作时就持续、肉眼可见地变化；单个很小装饰物缓慢漂浮、微粒/弱光或仅 hover/transition 不再足以单独兑现动态视觉，同时不规定固定动画技术、数量、层级或速度。
- 视觉扫描器增加“单一局部弱动效”识别：若整幅场景只有一个小局部动画，或唯一动画为长周期轻微 transform，会记录 `weak_visual_scenery_motion`，供后续真实输出纠偏。
- 动态视觉开关开启时，跟随主 API 的最终核对继续锁定 10.2.2 Visual Scenery，避免题材媒介词抢占主骨架；独立 API 请求链、美化、随机抽选与交互维修库均未改动。

## 1.2.58
- 可见语言规则改为“简体中文为主”，允许少量必要英文缩写、专有名词与风格性短词，不再要求所有可见词逐一中文化。
- 新增插件运行时语言比例检查：仅在英文显著接管主要可见文案时提示，不因 MVP/BPM/AI/API 等少量英文术语误报。
- 维修兔会对完成态兔子镜做轻量语言检查；命中时显示黄灯提示，但不会擅自改写正文或自动追加付费 API 请求。
- 全链路诊断新增中文字符、英文字母、英文词数量与英文主导判定，便于区分“少量英文点缀”和“整面英文界面”。
# 1.2.57

- 维修兔：修复“同一消息里其它镜面的控件掩盖当前镜面控件丢失”的诊断作用域问题；控件/内联事件损失改为按当前 summary 对应的原始镜面与当前 DOM 精确比较。
- 维修兔：当原始当前镜面仍有 checkbox/radio、但当前渲染镜面已被宿主剥离控件时，改走源码级安全重建当前镜面，而不是在缺少触发器的 DOM 上继续做无效交互兜底。重建会恢复被一同压平的 label 子结构，再由既有安全 onchange 解析器恢复交互，不执行模型原始 JavaScript。
- 维修库版本升级至 v2.14；未修改 Prompt、主题随机、Visual Scenery、独立 API 请求链或挨打猫逻辑。

## 1.2.57

- 维修兔内部维修库升级至 v2.13：跨父层 `:checked` 不再以“已安装兜底”直接视为修复成功，必须通过隐藏隔离副本的第二状态验证后才移除故障 finding。
- 修正隐藏副本验证复制了已修复 `style=...!important`、却没有复制 WeakMap 中修复前样式，导致取消 checked 时误报 `changed=0` 的问题；现在先清理可识别的持久化兜底痕迹，再在沙盒内重建可逆状态。
- 自动维修命中跨父层 checked 路线时会自动安排无点击、无事件派发的隔离验证；验证失败继续保持黄灯，不再显示“已验证修复”。
- 未修改 Prompt、主题/展现形式随机抽取、Visual Scenery、独立 API 请求链或挨打猫。

## 1.2.55

- 抽选随机源优先改用浏览器 `crypto.getRandomValues()`，受限环境自动回退 `Math.random()`；同一生成作用域仍保持同一次抽选，避免一轮内多次构建 Prompt 时自相矛盾。
- 修正主题家族完全等权造成的单条家族偏高：家族基础权重改为按有效条目数的 0.9 次方做温和规模校正；保留最近条目硬排除与父类/家族软冷却。
- 未修改主题文本、展现形式、Prompt 美化、Visual Scenery、独立 API 或交互链。
...
- 独立 API 外置成品挂载时自动安装高置信交互急救，而不再只做 ID 隔离；跨父层 `:checked`、radio 可逆返回、单向揭示等已存在急救路线在首次挂载与后续重挂载后都会重新绑定，解决“模型有控件/CSS但点不了”以及“维修后重挂载又失去监听器”。
- 维修后的本地成品若时间戳新于聊天 `chatMetadata`，同步时以新本地成品为准并反向修正聊天副本；`saveMetadata()` 改为串行后台保存，避免较慢的旧保存覆盖较新的维修/生成快照。
- 跨设备全量迁移/同步退出独立 API 的付费请求前置链：请求前不再扫描并保存整份聊天持久化数据；成功成品仍在生成后异步写入跨设备副本。
- `GENERATION_STARTED` 不再全局清空所有消息的 generation poll；新回复只替换“最新消息/占位”临时调度，上一条已经进入“正在生成中”的独立兔子镜继续拥有自己的稳定等待与请求生命周期，避免两条都显示生成中却只剩后一条真正请求。
- 保留 1.2.50 的 sibling `<style>` 外置迁移修复，并在外置重挂载后再次激活交互，避免 CSS/监听器在缓存恢复、DOM 替换或跨设备恢复后退化。
- 交付 ZIP 改回扩展文件直接位于压缩包根目录，不再额外套 `tototest-main/`；删除未被当前前端使用且缺少配套插件目录的 `SERVER-BRIDGE-INSTALL.txt`。当前副 API 使用 SillyTavern 自带 Custom Chat Completions 服务端转发，无需额外 server plugin。
- 维修兔内部版本更新为 v2.12。

# 1.2.52

- 保留现有 `presentationEmbodimentRule()` 与 `complexInteractiveCore()` 原文，不改已验证的展现形式落地、美化、色彩、材质与交互底盘。
- 从 `MEDIA_SELF_JUDGMENT_RULES` 仅接入一条最短媒介内容转译补强：把正文中的人物、物件、关系、变化或信息转成媒介原生部件、痕迹、状态与空间位置。
- `DYNAMIC_COMMITMENT_RULES` 改为按需接入：Visual Scenery 必读；普通展现形式仅在母本明确承诺持续动态、动画或倒计时时读取，并且单轮只注入一次。
- 精简 Visual Scenery 内部重复：移除固定 `@keyframes + infinite + 1 秒` 技术锁和重复动态宣告，保留场景优先、场景内有效交互、可恢复状态与正文正常流。
- 缩短 10.2.2 母本摘要；删除“亲密感官意味改为诗意化表达”。

# 1.2.50
- 修复独立 API 外置兔子镜丢失同级 `<style>` 的问题：模型把样式写在 `<toto>` 内、`<details>` 外时，1.2.48 外置渲染只取 `<details>`，导致 CSS 被丢弃，表现为原生 checkbox 暴露、动态场景退化成裸文字、`:checked` 交互与返回路径同时失效。
- 现在外置渲染会把同一成品片段中已经完成逐镜作用域处理的 sibling `<style>` 安全迁入对应 `<details>` 后再挂载；旧独立 API 缓存也会在重新渲染时走同一恢复链。
- 未修改主题/展现形式抽取、共同美化规则、Visual Scenery 取材规则或独立 API 配色冷却。

# 1.2.48
- 交互规范新增“非一次性主要交互必须可逆”硬要求：进入第二状态后必须能再次触发原对象或使用当前状态内的返回入口恢复初始界面；禁止唯一触发器永久消失、radio 选中后无取消路径。Visual Scenery 与独立 API 最终执行锁同步执行该自检。
- 通用交互急救新增 radio 可逆返回：记录每个 radio 组首次安装时的 checked 基线；用户再次点按当前已选项的 label 时恢复初始组状态。无默认选中则全部取消，有默认页则回到默认页，不编造任何新内容。
- 全链路诊断新增 `radio可逆返回` 计数与最近恢复状态，便于区分“同组 name 恢复”与“真正能回到初始界面”。
- 统一刷新运行时模块缓存版本到 1.2.48，避免热更新时混用 1.2.46/1.2.47 子模块。未修改主题/展现形式抽取、配色冷却与共同美化规则。

# 1.2.47
- Visual Scenery 重新放开回忆、幻想、未来畅想、可能性、梦境与象征化关系的取材，不再把“可辨认画面”误锁成只能复刻现实现场；同时继续硬挡抽象色块、渐变海报、无主体装饰组合冒充动态画面本体。
- 独立 API 配色纠偏从“只看主承载面明度”升级为“看最近真实成品的整体视觉色调家族”：综合主底盘的明度、冷暖、色相倾向、饱和度与整体观感识别连续重复，专门压制连续灰白、灰绿、灰蓝、淡冷白、低饱和雾面等近似底盘连发。
- 当整体色调指纹证据不足时，仍保留原有明度重复兜底，不让独立 API 因识别不稳而彻底失去配色冷却保护。

# 1.2.46
- 独立 API 成功兔子镜新增聊天级跨设备持久化：当前每条回复／Swipe 的成品同步保存到 SillyTavern 当前聊天 `chatMetadata`，`localStorage` 降为本机缓存；同一服务器上的手机、电脑或另一浏览器打开该聊天时可重新恢复同一面成品。
- 升级后打开旧聊天时，会把当前设备仍保存在旧 `rabbit_mirror_independent_outputs_v1`／owner lock 中的历史成品一次性迁移到聊天元数据；服务器已有记录或“重说中”墓碑时不反向用旧设备缓存覆盖。
- “重说”期间写入聊天级抑制墓碑，避免生命周期同步把旧 A 从另一设备缓存重新挂回来；成功后原位替换为新成品。维修兔对独立成品的持久修复也同步更新跨设备副本。
- 独立 API 组装上下文时显式排除兔子镜跨设备持久化字段，避免保存的 HTML 被重新塞进副 API Prompt；未修改共同美化规则、主 API Prompt、随机抽取或配色规则。

# 1.2.45
- Visual Scenery 恢复“完整动态舞台 + 主动画/环境动画”要求，并新增具体画面硬锁：抽象色块、渐变、光斑、线条与几何形只能辅助，不能冒充画面主体。
- 保留现实、回忆、想象、未来畅想、可能性与象征情境的自由取材，不把 Visual Scenery 限定为自然风景或当前现场复刻。
- Visual Scenery 再次强制至少一条场景内有效交互；hover/active 仅可辅助，不能单独算交互。独立 API 最终执行锁、视觉扫描与维修兔诊断同步取消“纯视觉免交互”例外。

# 1.2.44
- 独立 API 的 Visual Scenery 最终执行锁新增“去掉全部可见文字后仍必须是一幅完整动态画面”的硬验收，禁止用长段正文、纵排文字或说明面板充当主要画面；连续正文必须移到画布外正常文档流。
- 视觉扫描器新增动态画面“文字主导”与“固定画布长正文裁切”风险记录，下一轮会据真实成品触发针对性纠偏，不再把有 keyframes 就直接视为画面已经成立。
- 手机端新增高置信动态视觉正文越界急救：固定高度且 overflow:hidden/clip 的 Visual Scenery 若把长正文绝对定位到画布外，会在窄屏把越界正文安全复制到画布后的正常流，并仅在手机端隐藏被裁切的原副本；不改聊天正文。
- 自动安全巡逻也会执行上述高置信越界修复；维修兔全链路诊断新增“动态视觉长正文越界”计数。
- 通用 CSS 清洗补充 `transition:transform1.5s`、相邻 transform 函数粘连与渐变色标缺空格修复，避免动态场景因模型紧凑输出而丢失过渡/变形。
- 未修改主 API 注入主链、主题/展现形式母本检索策略、独立 API 请求 profile、A/B 成品锁与配色闭环。

# 1.2.43
- 修复 Visual Scenery 主画布 class 与逐镜 CSS 前缀失配：CSS 已变成 `rmc-*` 选择器、DOM 却残留原始 class 时，通用清洗链会在同一 scope 内按实际 style 做唯一别名回补。
- 独立 API 首次挂载前也执行同一套通用 class 一致性修复，避免主画布高度、定位、背景整段失效。
- Visual Scenery 纯视觉模式不再被维修兔误判为“缺少交互”。
- 清洗器补充高置信 CSS 粘连修复：`linearinfinite`、`infinitealternate-reverse`、`transparent70%`、`box-shadow: 0 020px`。

# 1.2.42
- 放松 Visual Scenery 生成硬约束：恢复“先让场景成立，再让最自然、最值得动的对象去动”的方向，不再强压前中后景清单、双动画套餐或外挂操作面板。
- Visual Scenery 重新允许纯视觉呈现；独立 API 最终执行锁、主注入规则与动态场景规则同步改为“交互可选、但若存在必须寄生于场景内部对象”。
- 视觉扫描器不再把 Visual Scenery 缺少交互直接判为风险，并下调了动态层级的验收门槛，减少把完整氛围场景误判成“不合格动态视觉”的情况。

# 1.2.41

- 修复手机端通用 Grid 维修误伤空间型展现形式：使用显式 `grid-column/grid-row`、模板区域或定位覆盖物构成的棋盘、地图、赛道、座位图等，不再折叠成普通单列。
- 空间型 Grid 在当前宽度足够时完全保留原列数、行数和坐标；确实超宽时只在自身启用局部横向滚动，避免棋子、事件格和绝对定位标记错位。
- 普通文章／卡片 Grid 仍会在真实窄屏溢出时按原逻辑单列化；不再仅因为存在固定 px/rem 轨道就预先强制折叠。
- 窄屏参考宽度现在同时受真实 viewport 限制，避免已经横向溢出的根容器反过来把 440px 手机误判成 640px 可用宽度。
- 未修改主 API Prompt、共同规则、展现形式库、独立 API 请求／配色闭环、挨打猫；原有 direct-ID onclick 与 self-mutation 安全交互恢复函数保持逐函数一致。

# 1.2.40

- 将独立 API 的配色闭环从“只防连续低明度”改为对称的主承载面明度家族重复冷却：最近两面真实成品若连续属于同一明度家族（低／中／高），第三面不得继续复用该家族。
- 判定只看主要承载面的整体明度；黑底白字、黑底绿字等仍属于同一低明度家族，白底黑字、白底粉字等仍属于同一高明度家族，不能靠换文字或强调色逃避。
- 不指定下一面必须变成哪一种明度、冷暖、彩度或具体颜色；只在连续两面相同后触发一轮，真实成品一旦换出该明度家族就自然解除，避免黑→白→中机械轮换。
- 继续只作用于独立 API 的成品历史与近输出纠偏；未修改主 API 共同 promptBuilder/storage、通用 outputSanitizer、visualScanner、展现形式落地、挨打猫主体及独立 API 请求/profile/A-B 成品锁。

# 1.2.40

- 最高强度复审 1.2.37 的独立 API 配色闭环：确认深色检测器虽已重新调用，但“连续两面后才纠偏”仍允许连续黑底，并且纠偏位于较早的 system Prompt，容易被后续长执行锁稀释。
- 改为独立 API 当前聊天内的真实成品闭环：最近一面只要被可靠识别为低明度主承载面，下一次独立请求即在最终执行锁之后加入一次近输出纠偏；若模型仍输出低明度，纠偏会继续保持，直到真实生成出非低明度成品后自然解除。
- 深色纠偏不点名具体色相，不要求冷暖或彩度轮换；局部暗部仍允许，只阻止低明度继续主导整面。配色家族重复提醒继续作为次级、单轮提醒。
- 独立配色历史只读取当前聊天，避免不同角色／聊天之间互相污染；独立重说历史现在保留 paletteFingerprint。
- 未修改共同 promptBuilder/storage 配色规则、通用 outputSanitizer 清洗逻辑、展现形式落地规则、挨打猫主体、独立 API 代理/profile/完整响应读取及 A/B 成品锁。

# 1.2.37

- 将“完全去掉避黑”调整为对称的一轮轻量配色家族冷却：只有最近两面实际输出被识别为同一明度／冷暖／彩度／主色家族时，下一轮才要求整体色彩底盘形成明显区别。
- 黑色、近黑、浅色、暖黄及其他颜色均不被永久禁止；冷却不指定下一轮必须变白、变蓝或使用任何具体色相，并且只作用于一轮，不形成机械轮换。
- 冷却要求不能只换强调色；主承载面的明度关系、冷暖重心或彩度结构至少有一项清晰改变。跟随主 API 与独立 API 使用同一原则。
- 合并旧跟随主 API 兔子镜恢复：热更新或消息 DOM 重绘后，会从当前消息源码、当前 Swipe 或更新前快照重新挂载旧跟随镜面，不重新请求 API、不改正文。
- 保留独立 API 成品锁及 1.2.31 的加载占位误判修复。

# 1.2.37

- 修复 1.2.29 引入的成品锁误判：独立 API 新正文出现“正在生成中”占位框后，该占位框会被当成已完成兔子镜，导致生成函数在发送请求前直接返回。
- 成品锁现在只信任 `data-rm-state=ready` 且不属于占位框的完整兔子镜；加载中、失败和重新生成状态均不能成为自动成品。
- 缓存与历史恢复会拒绝旧版误写入的加载／失败占位 HTML，使受影响正文能够正常发出一次合法请求并用真正成品覆盖错误记录。
- 保留 A/B 成品锁：同一正文第一份成功的自动结果仍不会被后续生命周期事件静默替换；只有用户明确重新生成时才允许替换。
- 未修改独立 API 的代理通道、兼容 profile、完整响应读取、Prompt、抽取池、配色、展现形式或手机端维修规则。
