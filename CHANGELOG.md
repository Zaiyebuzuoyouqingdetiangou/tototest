## 1.1.0-beta.14.60-test — 外置后内嵌跟随正文内容栏

- 修复“外置后内嵌”完成后虽然移动进消息，却被强制放到最外层 `.mes`，导致宽度按整条消息计算、明显宽于状态栏的问题。
- 内嵌锚点继续保持在 `.mes_text` 外部，绝不写入或序列化正文；但现在保留在 `.mes_text` 的同级内容容器中，并紧跟正文之后。
- 内嵌宿主宽度改为继承该内容栏的 100%，不再使用固定 560px 或人为左右减宽；手机与电脑均跟随各自消息内容区。
- 纯外置仍作为消息外的居中独立圆角框，保留原有最大宽度与悬浮间距。
- 未修改副 API 请求、缓存、Prompt、维修兔、挨打猫、正文身份或外框取色逻辑。
