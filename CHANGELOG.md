...
- 在 1.3.12 基础上仅微调外置兔子镜整体宽度：移动端响应式 gutter 从总计约 24px 增至约 32px，窄屏时左右各约 16px；标题栏、外框和内容区作为同一个兔子镜本体一起收窄，不给内部内容额外增加 padding。
- 桌面端外置最大宽度从 760px 微调为 740px，避免宽屏下仍显得略撑；仍保持自动居中。
- `inline` / 外置后内嵌完成后的 100% 正文容器自适配保持不变；未修改 Prompt、随机抽取、独立 API、外置一体化、历史顶层弹窗或维修兔逻辑。
- 本构建发布通道为测试仓库 `tototest`。
...
- 未修改历史数据、兔子镜生成、Prompt、随机抽取、外置宽度、外置一体化、独立 API 请求或 inline/外置后内嵌逻辑；本构建发布通道为测试仓库 `tototest`。
...
- 保留 1.3.8 之前的随机抽取、外置一体化、主视觉自适配、独立 API 与维修兔逻辑；本构建发布通道为测试仓库 `tototest`。
...
- 测试仓库通道：`https://github.com/Zaiyebuzuoyouqingdetiangou/tototest`。
