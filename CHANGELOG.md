## 1.1.0-beta.14.62-test — 跟随主 API 外置圆框与原位恢复

- 修复“跟随当前 API＋外置弹窗”的宿主未命中副 API 圆框 CSS，导致兔子镜裸露、铺满消息宽度的问题。
- 主 API 外置宿主现在使用与兔子镜主背景同系的圆角框、边框和阴影；不修改兔子镜内部样式，也不增加 API 请求。
- 主 API 外置宿主不再插到整条 `.mes` 之后，而是挂到 `.mes_text` 所在内容通道的安全同级锚点，避开消息按钮与 Swipe 区域造成的过低位置。
- 移出主 API 兔子镜前在原 DOM 位置留下隐藏标记；切回“正文下方”时精确放回原位，不再粗暴追加到 `.mes_text` 最末端。
- 对 beta.14.60 及更早已被移出的主 API 兔子镜，优先寻找遗留的空 `<toto>` 容器恢复位置，找不到时才回退到正文末尾。
- 未修改主 API Prompt、正文数据、Swipe、重说机制、副 API 请求、缓存、定时器或维修兔逻辑。
