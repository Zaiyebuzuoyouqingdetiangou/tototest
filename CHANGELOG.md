...
- 独立 API 的额外配色 guard 改为视觉家族 guard，手动重 roll 同样按视觉家族而非具体颜色避让，减少对颜色词的重复锚定。
- `shortVisualAvoidance` 不再重复回显具体配色名称；近输出短锁只在真实维度连续重复时提醒对应维度，降低 Prompt 中的颜色概念重复。
- 不修改外容器、sanitizer 主体、交互维修逻辑、母本索引、独立 API stream/retry/single-flight。
