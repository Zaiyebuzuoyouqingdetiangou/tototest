## 1.1.0-beta.14.59-test
- 修复正文结束后“立即预请求＋稳定版本再请求”的竞态：副 API 只在正文指纹稳定后启动一次。
- 同一聊天/mesid/Swipe 的正文版本变化时，按基础槽位取消旧版本请求与 pending，避免旧请求长期挂起。
- 独立请求增加 5 分钟超时收尾；占位兜底轮询降频并缩短，GENERATION_STARTED 事件标记不再永久锁死。
- “纯外置”保持消息外居中浮框；“外置后内嵌”完成后进入消息内部并使用全消息宽度、较浅阴影，仍保留同色圆角框。
- MESSAGE_RECEIVED／CHARACTER_MESSAGE_RENDERED 在移动端结束事件缺失时，可按精确版本补启动；旧占位框也会恢复排队，不再永久停在“正在生成中”。
- 外框渲染取色任务去重，并把实际 DOM 取色候选上限从 420 降到 220，减少同一帧重复布局读取。
