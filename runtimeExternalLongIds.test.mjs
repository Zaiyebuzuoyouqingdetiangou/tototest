...
test('five-face mixed and external-only long IDs retain exact attempts without entering builtin fairness', () => {
    const snapshot = fixture();
    const rows = new Map(snapshot.entries.map(entry => [entry.externalId, entry]));
    const seen = { external: 0, builtin: 0 };
    for (const mix of ['external-only', 'balanced', 'external-preferred']) for (let index = 0; index < 8; index += 1) {
        values.clear();
        context = { ...context, chatId: `long-external-mixed-${mix}-${index}` };
        const st = { ...structuredClone(settings.defaultSettings), enabled: true, autoRabbitMirrorInjection: true,
            mode: 'all', generationSource: 'independent', rabbitMirrorFaceCount: 5,
            themesMin: 1, themesMax: 1, formatsMin: 1, formatsMax: 1, avoidRepeat: false,
            blacklistEnabled: false, externalWorldBookRandomEnabled: true, externalWorldBookMixMode: mix };
        const selected = prompt.planRabbitMirrorPromptDetails(st, 'independent', null, context.chatId, {
            batchIdentity: { mesid: 0, swipeId: 0, sourceHash: 'original-mixed-source' }, chat: context.chat,
        });
        const result = prompt.renderRabbitMirrorPromptPlan(selected, new Map(selected.selectedExternalIds.map(id => [id, rows.get(id)])));
        const plan = result.batchPlan;
        assert.equal(plan.faces.length, 5);
        assert.ok(selected.selectedExternalIds.every(id => id.length > 128 && id.length <= 2048));
        if (mix === 'external-only') assert.equal(selected.selectedExternalIds.length, 10);
        const valid = new Set(plan.fairness.validFormatIds);
        assert.ok(plan.fairness.eligibleFormatIds.every(id => valid.has(id) && !id.startsWith('ext:')));
        assert.equal(storage.markPendingBatchAttempt(plan), true);
        const attempts = JSON.parse(values.get('rabbit_mirror_theater:generation_attempts:v1'))[plan.identity.chatKey];
        for (const face of plan.faces) {
            const stored = attempts.find(item => item.attemptId === `${plan.batchId}:${face.faceIndex}`);
            assert.deepEqual(stored.themeIds, face.combo.themeIds);
            assert.deepEqual(stored.formatIds, face.combo.formatIds);
            for (const id of [...stored.themeIds, ...stored.formatIds]) seen[id.startsWith('ext:') ? 'external' : 'builtin'] += 1;
        }
        assert.equal(storage.markPendingBatchAttempt(plan), true);
        assert.equal(storage.releasePendingComboBatch(plan), true);
    }
    assert.ok(seen.external > 0 && seen.builtin > 0, 'mixed runs cover both real source branches');
});
