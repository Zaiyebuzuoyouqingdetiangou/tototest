...
import { describeBatchPlanFailure } from '../src/externalWorldBook/errors.js';
...
test('local guard and batch-plan rejections are requestCount zero and never stage nostream', async () => {
    ...
    let releases = 0;
    const leaseSandbox = {
        describeBatchPlanFailure,
        markPendingBatchAttempt: (_plan, options) => {
            marks += 1;
            if (!markingAccepted) options?.onRejected?.('BATCH_REGISTRY_CAPACITY');
            return markingAccepted;
        },
...
globalThis.adapt=adapt;`, leaseSandbox);
    ...
    assert.throws(() => lease.consume(), error => {
        assert.equal(error?.code, 'RABBIT_MIRROR_BATCH_PLAN_REJECTED');
        assert.equal(error?.reasonCode, 'BATCH_REGISTRY_CAPACITY');
        assert.match(error.message, /本地仍保留 8 项生成任务登记/);
        assert.doesNotMatch(error.message, /同时生成数量已达上限/);
        return true;
    });
