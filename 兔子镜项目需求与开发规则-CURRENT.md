...
更新：2026-09-06。当前测试候选：1.5.20 RuntimeFix1；manifest/runtime 为 `1.5.20`，主 cache cohort 为 `1.5.20-runtimefix1`。本文是本包唯一 CURRENT；旧 README／CHANGELOG 历史段落和 `docs/multiface-phase2-design.md` 的早期方案不代替实际源码。此为测试候选，不是已完成真实宿主/模型/iPhone 验收的正式版。
...
- external pool singleton 的所有生产 import 必须使用同一 `?rmv=1.5.20-runtimefix1` specifier。保存／启用／停用／删除本地库时同步其轻量 ID 快照；通常 toggle/delete 不读取整库 raw，用户主动选择“重建索引并启用”除外。冷态 metadata 有有界 I/O 成本，不能声称开启状态总成本严格为零。
...
- 1.5.20 主 cache cohort 为 `1.5.20-runtimefix1`，变更的动态视觉 raw 模块同步加入该 cohort。导入/分类管理仍在用户点击后加载；生成时外部 raw 读取只在抽签计划已冻结且确实选中外部 ID 时发生，用户主动导入／重建是独立管理操作。启动轻桥不增加 raw、Regex 或禁词全聊天扫描；无新常驻 Observer/轮询。更新后必须整页刷新酒馆；不要以清空设置／冷却历史代替刷新。
