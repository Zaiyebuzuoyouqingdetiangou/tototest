...
更新：2026-09-06。当前测试候选：1.5.19 Usability1；manifest/runtime 为 `1.5.19`，主 cache cohort 为 `1.5.19-usability1`。本文是本包唯一 CURRENT；旧 README／CHANGELOG 历史段落和 `docs/multiface-phase2-design.md` 的早期方案不代替实际源码。此为自动化验证后的测试候选，不是已完成真实宿主/模型/iPhone 验收的正式版。

## 基线与授权

本轮源码基线为保留的 `RabbitMirror-1.5.18-Audit1C2-fixed.zip`，SHA-256 `18912d75cfa31ce32a773d6853cacffa6139ae42e90e5c76859dc1f9ebae48f2`。它承接上传的 1.5.17；1.5.16 仅作历史只读比较。输入包未覆盖；GitHub 保持只读，本轮未 commit、push、创建 branch／PR，也未写入正式或测试仓库。
...
- external pool singleton 的所有生产 import 必须使用同一 `?rmv=1.5.19-usability1` specifier。保存／启用／停用／删除本地库时同步其轻量 ID 快照；通常 toggle/delete 不读取整库 raw，用户主动选择“重建索引并启用”除外。冷态 metadata 有有界 I/O 成本，不能声称开启状态总成本严格为零。
...
- 1.5.19 主 cache cohort 为 `1.5.19-usability1`。导入/分类管理仍在用户点击后加载；生成时外部 raw 读取只在抽签计划已冻结且确实选中外部 ID 时发生，用户主动导入／重建是独立管理操作。启动轻桥不增加 raw、Regex 或禁词全聊天扫描；无新常驻 Observer/轮询。更新后必须整页刷新酒馆；不要以清空设置／冷却历史代替刷新。
