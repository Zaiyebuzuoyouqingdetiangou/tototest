...
- 视觉提示词编辑总开关默认仍为关闭；关闭时继续使用 1.3.20 原版视觉 Prompt 流程。
- 开启后，现有“额外视觉偏好”直接作为本轮明确视觉指令执行，不再按弱参考处理；“不希望出现的视觉”作为明确避用项。
- 用户明确偏好／避用项与通用视觉审美规则冲突时，优先采用用户本轮填写内容；展现形式本体与锁定工程规则仍不可覆盖。
- 不新增第二套 Prompt、不加入特定风格示例或词库。
- 性能冻结：不新增 MutationObserver、focus/resize/scroll 高频监听或输入框实时保存。
...
1.3.28 测试版的“个性化视觉提示词”使用显式总开关，默认关闭。用户可以预先编辑官方视觉审美层、额外视觉偏好与不希望出现的视觉，但只有勾选“启用视觉提示词编辑注入”后才会随生成请求发送。
