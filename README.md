# RabbitMirror / 兔子镜 1.4.30.4 TEST

## 1.4.30.4 TEST：配色结构冷却 + 主 API 维修兔身份识别

- 直接基于 1.4.30.1；外容器、sanitizer 主体、checked/radio 别名、母本、抽签与独立 API 请求链不做重构。
- 1.4.30.4 仅补一处独立 API 手动“重说”竞态：人工 force 重说跳过旧任务复用，先取消旧 flight，再沿原请求链新开一次；自动生成的 single-flight 去重不变。
- 配色冷却不再特殊合并蓝/青，也不永久偏好或排斥任何具体颜色。最近 3 面只作为短期历史；不能靠“蓝换紫/紫换绿”但继续复用同一白底/深底、冷暖、明暗、饱和度和对比结构来伪装成新配色。
- 跟随主 API 的维修兔识别从“标题文字唯一身份”改为“标题兼容线索 + RabbitMirror 自己的 `rmcss-*` scope 结构身份”。即使显示正则把 `【兔子镜：…】` 改成其它标题、或宿主剥掉 `<toto>`，最外层镜面仍可被找到。
- `兔子鏡` 与 `Rabbit Mirror` 标题也作为兼容识别；普通无 scope 的任意 `<details>` 不会因此被接管。
