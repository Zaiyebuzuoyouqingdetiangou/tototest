## 1.3.79 手机外置宽度：外壳跟随正文，作品本体保持自由

- external shell / details 仍跟随 SillyTavern 当前消息正文 lane；这个“跟随正文宽度”的设计没有取消。
- 撤回 1.3.78 对所有内部正文根节点强制 `width:100%` 的规则。票据、小舞台、设备屏幕、海报、固定尺寸视觉物等有意窄幅设计继续保持作者原宽度，不被统一拉满。
- 整面异常变窄继续由冷启动几何复测修正；真实超宽被裁切时只开放必要的横向滚动，不把作品压小。
