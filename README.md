...
本轮唯一代码基线为用户上传的 `tototest-main(1).zip`（实际版本 1.5.5）；旧 multiface step1 包仅作设计参考。当前规则见根目录唯一的 `兔子镜项目需求与开发规则-CURRENT.md`，第二阶段只读设计见 `docs/multiface-phase2-design.md`。

- 普通路线取消展现形式的 group 连带降权；最终选中主题有明确 `if` tag 时保留原 group 系数。exact 继续使用尝试记录，格式 family/group 使用正式成功记录；不改变主题规则、收藏倍率或母本内容，不做固定轮播。
- “个性化视觉提示词”页新增默认关闭的“增强视觉绘制”。开启仅增加一句媒介许可，不要求 SVG、不固定画风，不绕过净化；关闭时不增加 Prompt 字符。靠近 32,000 字符总预算时，开启所增加的 65 字符会挤占相同大小的上下文余量。
- 多面仅建立严格面数、带身份的批内抽签与 pending/history 基础。现有生成仍走单面，不发送多次请求、不挂载多个镜面。第二阶段尚未实现，不能把此版本当作可用的三面生成版本。
- 没有新增请求、定时器、Observer 或全聊天扫描。只新增一个设置控件的 change 保存监听，不把它说成“完全没有新增监听”。
- 原包 `libraryDataIntegrity.test.mjs` 的豆瓣条目与母本文字不一致仍需另案处理；本轮按不改母本的要求保留并报告，不删掉断言。原包净化器内联 foreignObject/部分内部属性策略差异未在本轮修改。

更新后请**整页刷新酒馆**。cache cohort `1.5.6-abc1` 防止模块 URL 混装，不代表未执行旧入口清理的同页热替换已获实机验证；不要调用“清空/重置”来代替刷新，以免清掉原冷却与反馈历史。

仅完成源码、自动化测试与随机分布模拟验证，尚未完成真实 SillyTavern、真实模型网络及手机实机验证。没有写入 GitHub、没有晋升正式版；输入 ZIP 保持原样。
