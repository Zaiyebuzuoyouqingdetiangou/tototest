# 1.3.38 TEST — 外置维修兔状态重建 / 视觉编辑最终优先

- 修复独立 API 外置镜面在缓存／重挂载后，checked 急救的 `!important` 状态被序列化保留、但 WeakMap 回滚记录丢失，导致 radio/checkbox 后续点击看起来完全无反应的问题。
- 仅在用户主动选择“交互维修”或“全部维修”（以及自动维修实际进入 interaction 步骤）时，对当前外置镜面执行一次状态重建：撤回救援拥有的残留 checked 内联状态，只重建当前已选分支，再复用既有 label 点击兜底与验证链。
- 不新增全局扫描、MutationObserver、focus/resize/scroll 监听或轮询；普通未点击维修的镜面不执行该重建。
- 视觉编辑开启时，将完整“用户可编辑视觉层”移动到更靠近最终输出契约的位置；最终短锁同时覆盖额外偏好与避用项，并明确要求改变整面作品的绘制语言，而不是只改标题、配色或图标。
- 视觉编辑关闭时仍保持原老流程，不追加本版视觉编辑规则。
