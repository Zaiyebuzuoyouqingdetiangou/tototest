...
1.3.24 测试版新增“🎨 视觉提示词编辑”。设置页可直接编辑从兔子镜原有 Prompt 中拆出的官方视觉审美层，并可追加个人视觉偏好与不希望出现的视觉。

- 跟随当前 API：保存后的视觉提示词随兔子镜注入一起发送给当前主模型；
- 独立 API：保存后的视觉提示词随兔子镜 system prompt 一起发送给独立模型；
- 不开放 HTML 输出协议、CSS 安全、手机兼容、交互可触发性、维修兔、冷却等工程规则；
- 为避免重现设置页卡顿，只在点击“保存视觉提示词”时写入设置，不在每次键入时保存；
