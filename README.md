...
独立 API 通过 SillyTavern 内置 Custom Chat Completions 后端以流式方式生成，减少长生成被网关 524 中断的概率。
...
> 测试版本：1.1.0-beta.14.22-test（原生外置折叠兔子镜、独立 API 与失败重试检测）
...
- 共同回忆资料来源：可在需要时生成回忆杀；
- 挨打猫：用于纠正兔子镜的美化效果；
- 维修兔 v1.88-test：检查和修复显示、排版及交互问题，包括原 CSS 引用唯一缺失 checked 控制类时的高置信恢复；
