...
本版以 1.4.30.33 候选成品为唯一源码基线，只修复长正文上下文预算、交互质量约束和用户主动拉取模型列表的慢响应问题；母本库、随机池、收藏／黑名单、维修兔、挨打猫、Touch Theater 专用规则、主／副 API 触发、世界书、标签隐私和单次付费请求边界保持不变。

- 修复单条长正文裁成约 8,000 字符后仍按裁剪前长度占用 12,000 字符聊天选择预算的问题；无标签过滤时不再误挤掉本可容纳的上一层前情。用户明确过滤的标签及包裹内容仍完全不外发，且过滤字符继续占用选择预算，不用更老聊天回填。
- 交互短锁不再把所有媒介压缩成“只需一条链”：内容存在多个值得探索的节点时要求多入口或连续阶段，单焦点媒介仍要求至少一条做深、做完整的交互链；不靠堆叠无关控件增加表面复杂度。
- “心跳回忆 GS 模式”综合父项及相簿、结局、ADV、萤火虫等兄弟形式不再因为说明文字提到“大接近模式”而误加载整套 Touch Theater 强规则；只有精确抽中对应专用 ID 时启用。
- 酒馆 Connection Profile 与手动接口的模型列表拉取仍各自只在用户点击时发送一次无付费状态请求，不自动重试；慢速中转的有界等待从 12 秒放宽至 30 秒，失败时继续保留该 Profile 已保存模型和当前手填模型，不清空选择。
- 测试版与正式版显示版本统一为 1.5；核心模块使用同一 `1.5-qualityfix1` cache-bust 闭包，避免热更新混装。
- 未新增 `fetch`、setInterval、MutationObserver、常驻轮询或全聊天扫描。仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实中转及同模型成品 A/B 实机测试；GitHub 未写入。
