# 1.3.34 TEST — 视觉偏好主导 / 纯外置标题壳贴合本体

- 将视觉编辑注入中的“额外视觉偏好”核心约束压缩为一句：**用户视觉偏好必须成为整面兔子镜可明确辨认的视觉主导，不得只做局部点缀；但不得抹掉本轮展现形式本体。**
- 修复独立 API 纯外置模式下，标题壳下方仍挂着一整条满宽舞台、真正窄本体悬在中间的问题。
- ready 后处理现在会优先识别居中且明显更窄的主本体（文档 / 纸页 / 设备 / 画面壳 / 主要内容承载块），并让外置宿主收缩到“标题壳宽度”和“主本体宽度”中的较大值。
- 纯外置详情容器不再强制走 `width:100%` 的整行舞台逻辑；正常全宽作品、场景型背景图与 follow 当前 API 外置模式不受影响。
- 不新增 MutationObserver、focus/resize/scroll 监听、轮询或全聊天扫描；既有不卡顿链保持不变。
