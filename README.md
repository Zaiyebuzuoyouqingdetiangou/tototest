# 兔子镜测试版 1.5.22 BatchFix1

基于原封不动保留的 1.5.21 InteractionFix1 安装包，修复外部母本连续生成多面时的一处发送前故障。

- 根因：上一轮抽过的外部条目被当成不可再选的硬排除，库虽然够生成五面，连续使用后仍无法建立计划。原创 45 条库在旧版第三轮稳定触发 `BATCH_SELECTION_INCOMPLETE`。
- 修复：优先避开历史条目；新条目不足时允许复用历史条目。同一批各面之间仍不重复，用户明确排除的条目绝不回用。不清空历史、不重建母本、不增加请求或自动重试。
- 1.5.21 的首次交互及本地维修修复保留。版本与模块缓存统一更新，避免新旧模块混用。

验证方法和边界见 `docs/batchfix-verification.md`。**这不代表手机上所有“计划失效／容量上限”分支均已实测解决；跨设备保存、手机发热与长延迟仍未验收。**

安装前备份旧扩展，完整覆盖扩展文件后整页刷新，确认版本为 1.5.22。不要清除站点数据、母本或冷却记录；不需要为了测试额外花费 API 额度。没有部署到在线酒馆或 GitHub。

# 保留的 1.5.21 InteractionFix1

这次是交互及本地维修保存修复包，不是已经验收通过的跨设备同步版。

- 修复新成品只隔离 ID、没有自动安装必要安全交互的问题；按当前面首次打开或首次内部操作初始化，后续折叠展开不重复扫描，不需要先点维修兔。
- 修复维修身份漏传消息序号，导致单面和多面都报 `chat metadata read-back mismatch`。修复的是本地存储及内存元数据，不是服务器确认。
- 维修成品与初始版本合计超出既有预算时先拒绝保存，保留旧的可恢复本地结果，不提高限额或清空缓存。
- 移除插件额外施加的强制动画播放，避免覆盖折叠暂停；细分多面计划失败原因，不自动重试或补请求。

**跨设备尚未实现、手机/电脑双向恢复尚未验收，手机温升及长延迟也未宣称解决。** 具体验收清单见 `docs/crossdevice-acceptance-pending.md`；本次验证见 `docs/interactionfix-verification.md`。

安装前备份原扩展，覆盖完整目录后整页刷新。不要卸载、清除站点数据或删旧手机缓存；新包不会替你同步已有成品。没有部署到在线酒馆或 GitHub。

# 保留的 1.5.20 RuntimeFix1

本次修订：

- 保留完整中文外部条目 ID，修复旧 128 字符校验误拒绝抽签计划；内置 ID 的限制不变，不要求重导或删除原库。
- 外部预取失败按同一操作结算后的精确正文身份显示错误，不再留下“正文正在更新”占位；错误展示固定代码，不展示母本正文。归属变化仍在请求前拒绝，不自动重抽或补请求。
- 导入工具进入浏览器顶层弹窗，关闭按钮和正文滚动区分离，支持安全区与窄屏；入口名称统一为“🚫 禁词与文字替换”。
- 同一回复未改动时复用交互源码签名；展开时先筛选潜在裁字区域，避免普通文本做全量布局测量。内嵌标题触控高度至少 48px，相邻面间距 24px。
- 动态视觉多面不再混入非场景的通用页面交互规则，去掉“画面外展开正文”的冲突许可；标题句式随媒介变化，不增加固定样板或新的请求。
- `unclosed-raw-text` 仍是结构不完整，不能凭空补全。仅上游明确报告长度终止时建议调整输出上限；合格面保留、不自动补发的规则不变。

验证范围及待验收项目见 `docs/runtimefix-verification.md`。先备份旧扩展，再覆盖完整目录并整页刷新；不要清除站点数据、母本或冷却记录。没有更新在线酒馆或 GitHub。

## 保留的 1.5.19 功能

当前规范以本包 `兔子镜项目需求与开发规则-CURRENT.md` 为准。以下旧版本小节仅作历史记录。

- 高级设置分别提供外部世界书与禁词／文字替换入口。本地导入直接选择 JSON 文件；强分类线索自动识别，弱线索支持批量采纳。
- 母本已保存与抽签索引状态分开显示；旧库索引需要用户主动重建，不在酒馆启动时扫描原始条目。
- 多面结果按面检查和保留，失败面显示原因，不自动补请求。独立 API 支持定向重试；跟随模式仅对可信留存批次的失败面，使用当前 Chat Completion 主 API 的 `generateRaw` 接口手动补一次。接口缺失、原抽签记录缺失或归属不明时不发送。
- 点击快照复用预算检查结果；跟随内嵌／外置转换保留控件状态并重建净化证明。
- 动态视觉不再限定单热点／单显隐链；大接近要求可辨认的完整人物轮廓。未提高输出额度或温度，所有面仍共用一次请求的输出上限。

这不是“所有截图和手机问题均已实机修复”的声明：真实云酒馆／TT／iPhone、模型美学、温升以及复杂裁字仍需目标环境验证。跟随模式普通成功面的重新生成不在本次失败面补生成范围内。

先备份旧扩展，覆盖完整目录后整页刷新酒馆；不要清除站点数据或冷却历史。没有对 GitHub 或在线酒馆进行发布。

# 历史：兔子镜测试版 1.5.9

## TerminalErrorFix1 / 1.5.9（测试候选）

本轮以用户上传的 1.5.8 安装包实际源码为唯一基线，只修复独立 API 终止错误在被动同步中被通用“正文变化”提示覆盖的问题，并补充与当前 chat／mesid／Swipe／sourceHash 绑定的无正文诊断。

- 已发送的一次请求若随后在空流、多面解析、净化或质量门失败，精确错误壳会继续保留；被动同步不得把它改写成未经证明的“正文随后又发生了变化”。
- 只有存在明确的正文替换证据时才使用“正文变化”提示；没有精确错误且没有替换证据时，使用诚实的“未形成可恢复完整成品”兜底。
- 保持每条目标消息最多一次付费请求，不自动重试、不切换模型、不补发缺面；本轮未修改多面整批失败语义。
- 最近请求诊断增加当前消息身份与终止阶段，不保存聊天正文、Prompt、密钥、响应正文、reasoning 或世界书正文。
- cache cohort 更新为 `1.5.9-terminalerror1`。原基线 `libraryDataIntegrity.test.mjs` 的母本双库不一致失败原样保留，本轮未改母本。

仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实独立 API／Connection Manager、真实模型网络与 iPhone Safari 实机验证。GitHub 未写入，输入 ZIP 未覆盖。

以下 1.5.8 小节为历史版本说明。

# 兔子镜测试版 1.5.8

## VisualStreamFix8 BoundaryFix1 / 1.5.8（测试候选）

本轮以逐文件冻结的 1.5.7 Multiface5 安装包为唯一源码基线，集中修复用户实测中的文字裁切、流式卡顿、等待／消失、独立 API 误报、跟随外置多面、自然返回、视觉趋同和 Token 表意；不改母本库、随机池、收藏／黑名单、标签／世界书选择、Touch Theater 或 12,000／20,000／32,000 字符预算。

- 轻量 bridge 在正文 START 时只记录 owner，不再同时解析约 2 MiB 重运行图；正文准确完成／终止后才以 120ms 合并唤醒。流式期间所有模式跳过逐 token DOM 同步，冷恢复复用完成时间、正文 hash 与 revision，不重新叠加旧退避。
- 连续发送第二条时，第一条已完成 intent 不再被删除；每个完成目标分别消费并恢复。同聊天、楼层、Swipe 与正文指纹继续隔离。
- 独立 API 的 Profile／上下文／批次／大小预检失败正确显示“本轮未发送、requestCount=0”，不再包装成网络断流或准备 nostream。真正请求仍每个操作最多一次，无自动重发。
- 修复独立 API 多面执行锁与安全检查边界名不一致，导致请求在发送前必然被拒绝的问题；单面与 2～5 面现在共用一套唯一、有序、完整闭合的安全边界，仍在付费请求凭证消费前拒绝缺失、重复或错序证据。
- Connection Manager 长流只对新增字符串做增量字节计数；未用于正文的隐藏非字符串状态每帧最多检查 512 项、64 层、63 个字段，验证后不保留。过深或持续膨胀会在响应边界安全停止，不伪装成网络断流，也不会提示 nostream 重试。
- 跟随当前 API 的多面外置模式会等每面净化 proof 与整批 commit 成功，再按精确消息身份外置；不依赖移动宿主可能滞留的 `isGenerating` 弱标志。
- 增强视觉补上 360px 正常流、行高与末行防裁切要求；当前最新、已完成且已展开的每个 face 最多做一次有界高置信检查（单批最多 5 次），只在存在独立、简单、安全的裁切祖先时自动解除裁切。行数限制、低行高和可能属于交互的结构交给维修兔判断，不扫描历史、不联网。
- 多面先冻结不同亮度、色系、材质、外轮廓、阅读路径与交互家族；除内容全部硬要求黑暗外至少一面明亮，禁止统一退化成深黑矩形卡或三按钮换皮。动态场景保持一主一辅两层明显运动，折叠面暂停后台动画，减少持续动画工作量；真实温升仍待目标手机实测。
- 不再注入突兀的通用“↶ 返回初始页”按钮，历史遗留按钮也会清理；每个非终局第二状态必须把返回融入媒介内部的自然对象／动作，异常恢复仍由维修兔完成，鼠标、触摸与键盘切换前快照保留。
- Token 卡按当前来源明确显示“跟随正文 API／独立 API”“最近／历史”和“Prompt 本地估算、非服务商账单”。独立项是请求消息内容字符合计，跟随项是兔子镜待注入 Prompt。
- 跟随模式不新增独立模型请求；独立模式在正文后再发一次兔子镜请求，1～5 面共享该请求。首次冷缓存仍可能下载同源模块，真实家庭网络、热量与视觉质量必须在目标 SillyTavern／设备用 HAR、服务端日志和实机继续验收。

更新后请**整页刷新酒馆**。cache cohort `1.5.8-visualstream8-boundary1` 用于避免覆盖安装后继续命中早先的 1.5.8 模块；不要用清空设置或冷却历史代替刷新。

原包 `libraryDataIntegrity.test.mjs` 的豆瓣 structured raw 与母本文字不一致仍原样保留，未删除或放宽旧断言。输入 ZIP 未覆盖，GitHub 未写入。

以下 `Multiface5 / 1.5.7` 小节记录旧版历史行为，其中通用“返回初始页”已被上面的 1.5.8 自然返回规则替代，不代表现行实现。

## Multiface5 / 1.5.7（测试候选）

本轮实际源码基线为 1.5.6 ABC1 候选；`兔子镜-multiface-step1(1).zip` 与旧第二阶段文档只作设计参考。现行行为以根目录 `兔子镜项目需求与开发规则-CURRENT.md` 和实际源码为准。

- “生成与抽取”可选普通单面或 2～5 面。无论跟随当前 API 还是独立 API，一轮只发送一次生成请求；共享全局规则，但每一面使用自己的抽取结果，后面各面避开本批前面已随机抽中的精确主题／展现形式。
- 模型必须返回 N 个平级、完整且互相独立的 `<toto><details>…</details></toto>`。每面标题、正文和 UI 构思独立；每面分别拥有维修兔、挨打猫、收藏／黑名单抽签记录与交互初态，不把多面塞进同一个 `<details>` 或 `<toto>`。
- 多面结果在一次响应结束后整批校验、逐面离线净化、一次提交；缺面、嵌套、跨面闭合、重复面、身份过期或预算超限都不会自动补请求。用户主动重说某一面时只替换那一面，其他面保持原 DOM 与状态。
- “增强视觉绘制”已移到“动态视觉场景”旁。两者可以同时开启：动态视觉决定场景型展现，增强绘制补足构图层次、材质、光影和可逆互动；只增加一份短规则，不按面复制，也不会增加网络请求。
- 塔罗／牌阵／西方神秘学抽中时，Prompt 要求使用兔子镜既有白名单 `https://gfx.tarot.com/images/site/decks/rider/full_size/0.jpg`～`77.jpg` 实体牌图和中文 `alt`；跟随与独立成品都会在净化后校验，缺图或错误外链不冒充成功。
- 兔子镜内真实交互后提供“↶ 返回初始页”；维修兔的“回到交互前”继续按当前面保存与恢复。鼠标、触摸和键盘激活均在状态改变前捕获初态。
- 独立 API 已接受的完整结果，不会因为随后缓存写入或 DOM 后置复核短暂未命中而反报失败或清除成品。HTTP 200 中若同时带 provider error 和看似完整正文，仍按错误处理且不自动重发。
- 1 面仍保持旧单面输出协议；历史设置缺少多面字段时默认 1 面。12,000 字符聊天正文、20,000 字符上下文、32,000 字符完整请求上限、单次付费 lease、SSE／NDJSON／非流式与 DeepSeek reasoning 排除均保留。
- 原包 `libraryDataIntegrity.test.mjs` 的豆瓣 structured raw 与母本文字不一致仍按原样保留，未删除或放宽旧断言；最终自动化数字以本轮验证报告为准。

更新后请**整页刷新酒馆**。cache cohort `1.5.7-multiface5` 用于避免覆盖安装后混装旧模块；不要用清空设置或冷却历史代替刷新。

仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实主／副 API、真实模型、PC／Android／iPhone 实机与视觉质量验收。没有写入 GitHub、没有晋升正式仓库；输入 ZIP 保持原样。

下面是历史版本说明，不代表本轮新增改动或现行验收结论。

## VarietyFix1 / 1.5.5

本版以 QualityFix5 / 1.5.4 的实际源码为基线，修复安卓本地酒馆在独立 API 成功结算时镜面随宿主 DOM 替换消失的问题，并扩充主题／展现母本与近期抽取多样性；不改付费请求单次约束、主／独立 API 边界、标签与世界书、Token 口径或 12,000／20,000／32,000 字符上限。
