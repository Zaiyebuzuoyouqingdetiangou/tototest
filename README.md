# 兔子镜测试版 SecurityFix5 TagScan1

## SecurityFix5 TagScan1 / 1.4.30.21

标签过滤窗口现在可以点击“扫描当前聊天标签”。扫描只查看当前已加载聊天中的可见正文，列出自定义标签及出现次数；扫描结果不会自动启用，必须由用户勾选并保存，才从下一轮副 API 上下文副本中移除对应标签区块。

- 不扫描模型 reasoning 字段、未渲染历史、隐藏节点、兔子镜成品、工具入口或设置窗口，不发送或保存正文内容。
- 普通布局标签与代码块里的示例标签不会进入候选；真实自定义标签以及正文中可见的转义／二次转义标签可以被发现。
- 长聊天使用分帧扫描和硬上限；关闭窗口、再次扫描或切换聊天会立即使旧扫描失效。
- 保留 SecurityFix4 ContextSpeed1 的模型无关快速通道、单 Response 流式接收、请求内小型正文缓存、标签过滤和单次付费请求边界。
