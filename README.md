## 1.3.80 手机纯外置正文 lane 共识修复

- 保留 1.3.77 的 pure-external「跟随正文宽度」方向，但不再只相信当前这一条消息某一帧的 `.mes_text` 宽度。窄屏下若当前正文显著窄于附近已渲染普通消息的稳定正文比例，会使用附近消息的中位正文 lane 作为高置信纠偏；正常一致的 `.mes_text` 仍按原逻辑使用。
- 修正冷启动定点复测实际只跑一次的问题：420ms 结果若与首测相同，旧逻辑会直接停止，1500ms 第二次复测根本不会执行。现在两个定点复测都会执行，因此「420ms 时仍窄、稍后主题/字体才稳定」也能自动纠正。
- 邻近共识最多读取当前消息前后 4 条有效消息，并要求至少 2 条、比例离散度足够小才采用；不做全聊天扫描、不新增 observer/listener/轮询。
- 不改内置兔子镜宽度、不强制内部作品满宽、不修改维修兔移动排版规则；真正超宽内容仍由既有横向裁切急救负责。

## 1.3.77 移动端横向裁切热修复

- 撤回 1.3.76 会把正常固定宽度视觉内容压窄的 `max-width:100% / min-width:0` 内容改写。
- 横向裁切急救现在只在高置信真实裁切时开放最近祖先的横向滚动，保留模型原本的正文与视觉舞台宽度。
- 保留 1.3.75 的冷启动几何复测、🎲 按钮保留与本地状态修复，以及 1.3.76 的 transient/装饰过滤设计。

## 1.3.74 交互移动目标与维修工具重建

- 对动画持续移动、命中区很小且已验证由 checked 打开第二层内容的 label，增加 pointer 级点击补偿；正常 click 成功时不重复切换。
- 修复维修兔 clone+replace 后，以 `<details>` 本身作为 refresh scope 时根节点发现器漏掉 scope self，导致 🐇 / 🐈 / 🎲 运行时工具无法立即重建的问题。
- 不修改 Prompt、独立 API 请求、抽签池、黑名单规则、调色盘冷却，也不新增轮询或全聊天扫描。

## 1.3.73 视觉编辑种子补全与完成度地板

- 简单视觉偏好按设计种子处理，自动补足构图、层级、光线、排版、材质细节与交互第二状态；更完整的视觉草图/规格只补缺口，不反向改写用户要求。
- 完成度下限仅在开启视觉提示词编辑时生效，并明确“完成度不等于复杂度”：允许极简，但不能退化成通用卡片或靠堆装饰假装高级。
- 设置页改用完整但不冗长的视觉示范句，同时说明只写“毛玻璃”“粉嫩清新”也可以，系统会自动把短偏好补成完整设计。
- 不扩大提示词开放范围；输出协议、安全、手机适配、交互兼容、冷却和维修兔规则仍由插件锁定。

## 1.3.71 可编辑视觉 Prompt 上限收紧

- 个性化视觉提示词继续只开放成品视觉层，不开放输出协议、HTML/CSS 安全、手机适配、冷却与维修契约。
- 修正“只填写避用项”时近输出执行锁会产生自相矛盾的“避用项必须主导整面作品”问题。
- 独立 API 模式的 Token 面板现在也能看到兔子镜自身规则与可编辑视觉层开销；聊天/角色卡/世界书上下文单独显示字符数。
- 维修兔 class-local/generic-local 的 `:checked ~ target` malformed fallback 只允许命中控件后方区域，不再误带动控件上方同类元素。

## 1.3.51 手机纯外置宽度

手机纯外置现在优先跟随当前消息实际渲染的正文区域（`.mes_text`），不再用固定 `vw` 猜宽度；电脑端仍保留 PC-only compact-shell，避免宽屏被正文无限拉宽。

## 1.3.50 手机纯外置宽度

窄屏独立 API「轻壳外置」直接使用 `84vw` 作为正文通道宽度（左右约各 `8vw`），不再依赖外置父级的百分比宽度或首帧 lane 像素快照；小屏手机会随 viewport 连续缩放。PC 外置几何与 compact-shell 不变。
...
当前测试版本：`1.3.96 TEST / RC`。测试仓库扩展显示名称固定为“兔子镜测试版”，用于与正式版“兔子镜小剧场”区分。

独立 API 的模型列表属于**可选辅助能力**：若供应商不提供 `GET /models`、上游拒绝模型枚举，或 SillyTavern 只能返回 `error:true`，兔子镜会显示真实可获得的错误信息，并保留手动填写的 model ID；模型列表失败不等于生成接口一定不可用。
