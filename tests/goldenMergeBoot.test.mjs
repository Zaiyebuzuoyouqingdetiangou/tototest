...
assert.match(index, /GOLDEN_MERGE_VERSION\s*=\s*'1\.4\.9-externaldiag1-securityfix5-tagscan1'/);
...
assert.match(index, /import\('\.\/src\/ui\.js\?rmv=1\.4\.9-tagscan1'\)/);
assert.match(index, /import\('\.\/src\/externalDiagnostics\.js\?rmv=1\.4\.9-tagscan1'\)/);
