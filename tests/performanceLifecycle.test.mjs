...
assert.match(visual, /commitRabbitMirrorFollowBatch\(set\.batchId, chat, scans, \{\.\.\.set\.owner,partial:scans\.some\(scan=>scan===null\)\}\)[\s\S]{0,500}FOLLOW_MULTIFACE_COMMITTED_EVENT/, 'successful proof+commit must emit one exact-owner local event');
...
assert.match(index, /\.\/src\/ui\.js\?rmv=1\.5\.20-runtimefix1/, '1.5 UI parent must use its dedicated cache key');
...
assert.equal(manifest.js, 'index.js?rmv=1.5.20-runtimefix1');
